...
선택 여부를 바꾸려면 기존 기록을 덮어쓰지 말고 판단 변경을 새 리뷰 기록에 남깁니다. 현재 CLI에서 rejected를 다시 선택하는 승격 기능은 없습니다. 원본 파일이 유일한 참조가 아니었다면 모든 실제 참조를 기록합니다. 참조와 raw는 바이트 그대로 복사합니다. CLI가 작성하는 파일 참조 필드는 상대 경로와 SHA-256을 사용합니다. 프롬프트·메모·분석·추가 레이어 필드와 이미지 메타데이터는 자동 익명화하지 않으므로 공유 전에 별도로 확인합니다.
...
코드와 제작 사례를 구분합니다. 고객/게임마다 `jobs/`를 분리하고 해당 자료의 보관·공유 권한을 관리합니다. 기본 gitignore는 이미지·PSD·번들·ZIP을 제외합니다. 공개 사례로 선택한 성공 적용 사례 2건과 제작 실험 3종만 `case-studies/furuyoni/` 경로에서 예외로 추적하며, 해당 폴더의 ASSET-RIGHTS 안내를 따릅니다. 사례 원본·생성물·PSD에는 코드의 MIT 라이선스가 적용되지 않습니다.

## 입력 파일의 신뢰 범위

JSON의 입력 경로는 JSON 위치 기준으로 해석하며 절대 경로나 상위 폴더의 파일도 가져올 수 있습니다. 특히 `processing_files`는 이미지가 아닌 파일도 그대로 복사합니다. 다른 사람이 제공한 JSON을 실행하기 전에 모든 입력 경로를 검토하세요. 작업 폴더의 출력은 해석된 경로가 작업 폴더 내부인지 검사하지만, 입력 파일을 격리하는 샌드박스는 아닙니다. 한 폴더에는 한 작업자만 쓰고 동시 변경을 피합니다. 자세한 범위는 [보안 정책](../SECURITY.md)을 따릅니다.

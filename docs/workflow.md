...
코드 저장소에는 스킬·일반 도구·샘플 스펙·합성 테스트만 넣습니다. 고객/게임마다 `jobs/`를 분리하고 해당 자료의 보관·공유 권한을 관리합니다. 기본 gitignore는 이미지·PSD·번들·ZIP을 제외합니다. 예외적으로 포트폴리오를 공개하려면 권리와 허용 범위를 확인한 별도 자료로 취급합니다.

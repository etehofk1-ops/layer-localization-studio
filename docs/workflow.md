...
코드와 제작 사례를 구분합니다. 고객/게임마다 `jobs/`를 분리하고 해당 자료의 보관·공유 권한을 관리합니다. 기본 gitignore는 이미지·PSD·번들·ZIP을 제외합니다. 사용자가 요청한 비공개 저장소의 실제 사례 세 개만 `case-studies/furuyoni/` 경로에서 예외로 추적하며, 해당 폴더의 ASSET-RIGHTS 안내를 따릅니다. 사례 원본·생성물·PSD에는 코드의 MIT 라이선스가 적용되지 않습니다.
